...
class Vector2(object):
    ...
    def grow(a,l):
        aAngle=math.asin(a.y/a.get_length())
        abx=a.x*l/a.get_length()
        aby=l*math.sin(aAngle)
        return Vector2((abx+a.x),(aby+a.y))

    def rotate(self,angle):
        rAngle=(angle/180.0)*math.pi
        bx=self.x*math.cos(rAngle)-self.y*math.sin(rAngle)
        by=self.x*math.sin(rAngle)+self.y*math.cos(rAngle)
